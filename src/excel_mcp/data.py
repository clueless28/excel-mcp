...
def filter_rows_by_value(
    ...
    """Filter rows based on value in a specific column."""
    filtered = []
    for row in data:
        if len(row) > column_index:
            match = row[column_index] == value
            if (include_matches and match) or (not include_matches and not match):
                filtered.append(row)
    return filtered
...
def delete_rows_by_value(
    ...
    """Delete rows that contain a specific value in the given column index."""
    return [row for row in data if len(row) <= column_index or row[column_index] != value]
